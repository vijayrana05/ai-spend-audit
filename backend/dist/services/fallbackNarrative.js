...
exports.buildFallbackNarrative = buildFallbackNarrative;
function money(n) {
...
function buildFallbackNarrative(auditResult) {
    ...
    const topFindings = top.map((it) => ({
        ...
        detail: typeof it?.explanation === "string" && it.explanation.length > 0
            ? it.explanation
            : "Review your current plan vs your actual seat count and usage to avoid paying for unused capacity.",
        impactUsdMonthly: typeof it?.estimatedSavingsMonthlyUsd === "number" ? it.estimatedSavingsMonthlyUsd : undefined,
    }));
    const exec = (typeof monthly === "number" && monthly > 0)
    ...
    return {
        ...
        disclaimer: "Estimate only. Verify actual charges against invoices and the vendor pricing pages before making purchasing decisions.",
    };
}
