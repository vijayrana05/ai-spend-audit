...
exports.GEMINI_MODEL = process.env.GEMINI_MODEL ?? "gemini-3-flash-preview";
const OUTPUT_SCHEMA = {
    type: "object",
...
async function generateNarrativeSummary(input) {
    const apiKey = required("GEMINI_API_KEY");
    const genAI = new generative_ai_1.GoogleGenerativeAI(apiKey);
    const model = genAI.getGenerativeModel({ model: exports.GEMINI_MODEL });
    const prompt = input.mode === "repair"
        ? input.repairPrompt
    ...
    try {
        ...
        throw new Error(`Model did not return valid JSON (model=${exports.GEMINI_MODEL}). Raw text: ${text.slice(0, 400)}`);
    }
    return parsed;
}
